...
basicTest :: Int -> IO ()
basicTest frameCount = runContextT GLFW.newContext
        (ContextFormatColorDepth RGB8 Depth16)
        (initRenderContext [xAxis,yAxis,zAxis,plane] >>= mainloop (frameCount, 0))
...
projectLines :: forall os. Shader os (ContextFormat RGBFloat Depth) (ShaderEnv os) (FragmentStream (V3 FFloat, FFloat))
projectLines = do
...
-- TODO: Candidate for removal; just "draws" the shader result w/o adding any other behavior
initRenderContext
